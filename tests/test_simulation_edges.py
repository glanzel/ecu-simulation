...
def test_ecuj_bundle_at_least_floor():
    ...
    cfg = SimulationConfig(
        ecumenge_ziel=1.0,
        random_seed=1,
        start_nutzung_anteil_budget={k: 0.45 for k in BOUNDARY_KEYS},
    )
...
def test_demand_never_exceeds_vej_default_config():
    ...
    cfg = SimulationConfig(ecumenge_ziel=1.0, random_seed=2)
    annual_co2 = 1.03**12
...
def test_high_start_demand_still_bounded():
    ...
    cfg = SimulationConfig(
        ecumenge_ziel=1.0,
        random_seed=4,
        start_nutzung_anteil_budget={k: 0.95 for k in BOUNDARY_KEYS},
    )
    cfg.price.preis_bump = 1.05
